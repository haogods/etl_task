...
class LgEtlStore(CBaseStore):
    def __init__(self, stage='raw'):
        CBaseStore.__init__(self)
        self._stage = stage
        self.raw_db_name = "jd_lagou_raw"
        self.measure_db_name = "jd_lagou_measure"
        self.raw_coll_name = "jd_raw"
        self.measure_coll_name = "jd_measure"

        self.pymongo_raw_client = pymongo.MongoClient(self.cmgClient.jd_lagou_raw_store_mongo_url)
        self.pymongo_measure_client = pymongo.MongoClient(self.cmgClient.jd_lagou_measure_store_mongo_url)

    def create_client(self, **kwargs):
        if self._stage == 'raw':
            return self.cmgClient.jd_lagou_raw_store_cmongo_client(self.raw_db_name, self.raw_coll_name)
        if self._stage == 'measure':
            return self.cmgClient.jd_lagou_measure_store_cmongo_client(self.measure_db_name, self.measure_coll_name)
