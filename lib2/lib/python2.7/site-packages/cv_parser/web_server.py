...
class CvParserHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if "cvfile" in self.request.files:
            ...
            if  re.search("智联卓聘",source_text):
                result = cv_parser.cv_parser_highpin.parser(htmlContent=source_text)

            elif re.search("liepin",source_text):
                result = cv_parser.cv_parser_liepin.parser(htmlContent=source_text)

            elif re.search("zhaopin.com",source_text):
            ...
            elif re.search("51job",source_text):
