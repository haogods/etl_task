...
class CvParser58(CvTopParser):
    ...
    def preprocess(self,htmlContent=None,fname='./data/100.html',url=None):
        ...
        self.result["cvFrom"] = "58tongcheng"
    ...
    def regular_cert(self):
        ...
        certsoup = ""
        for field in self.field_list:
            if re.search(u"证书",field.find("h2","field-title").get_text()):
                certsoup = field
                break
    ...
    def regular_private(self):
        ...
        for field in self.field_list:
            if re.search(u"我的亮点|介绍|简介",field.find("h2","field-title").get_text()):
                if field.find("ul","tags-list"):
                    res["keywords"] = [ tag.get_text().strip() for tag in field.find("ul","tags-list").find_all("li") ]
    ...
    def regular_other(self):
        ...
        if soup and soup.find("p"):
            res["selfIntro"] = soup.find("p").get_text().strip() if soup.find("p") else ""
...
if __name__ == "__main__":
    ...
    # print(json.dumps(result,ensure_ascii=False,indent=4))
    htmlContent = codecs.open('/home/zhangzq/PycharmProjects/untitled/data/cv_58/185.html','rb','utf-8').read()
