...
class ETLTask(BaseTask):
    def __init__(self, owner, queue_size, thread_cnt):
        ...
        self.jd_parse_client = self.thrift_client.jd_parse_server_client
        self.jd_store_client = self.thrift_client.jd_store_server_client
    ...
    def parse_measure_jd(self, item):
        try:
            ...
            jd_raw = self.jd_parse_client.analyzeHtml(pageContent, self.owner)
            jd_measure = self.jd_measure_client.measureJd(jd_raw)

            self.jd_store_client.setJdRaw(jd_raw)
            self.jd_store_client.setJdMeasure(jd_measure)
